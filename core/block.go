...
// blockNullables — nullable-поля при загрузке блока из БД (могут быть NULL).
type blockNullables struct {
...
func applyBlockNullables(block *Block, n blockNullables) {
	...
	if n.version.Valid {
		block.Version = uint32(n.version.Int64)
	}
	if n.size.Valid {
		block.Size = uint64(n.size.Int64)
	}
	if n.txCount.Valid {
		block.TxCount = uint32(n.txCount.Int64)
	}
	if n.gasUsed.Valid {
		block.GasUsed = uint64(n.gasUsed.Int64)
	}
	if n.gasLimit.Valid {
		block.GasLimit = uint64(n.gasLimit.Int64)
	}
	if n.difficulty.Valid {
		block.Difficulty = uint64(n.difficulty.Int64)
	}
}
...
// GetBlockIndexByID возвращает номер блока в цепи (blocks.height) по внутреннему id.
// Используется в API транзакции для block_number; GET /block/:number ищет по height.
func GetBlockIndexByID(ctx context.Context, pool *pgxpool.Pool, blockID int64) (uint64, error) {
	if pool == nil || blockID <= 0 {
		return 0, nil
	}
	var height uint64
	err := pool.QueryRow(ctx, `SELECT COALESCE(height, index) FROM blocks WHERE id = $1`, blockID).Scan(&height)
	if err != nil {
		return 0, err
	}
	return height, nil
