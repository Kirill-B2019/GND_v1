...
- `Transfer` — перевод токенов (ERC-20 совместимое). Токены GND-st1 при вызове `transfer()`/`transferFrom()` записывают событие в таблицу `events` и рассылают его подписчикам WebSocket (порт 8183).
- `Approval` — одобрение расходования токенов. При вызове `approve()` для GND-st1 событие сохраняется в `events` и передаётся через WebSocket.
