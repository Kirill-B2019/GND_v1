...
Если занят только порт **8183** (WebSocket), нода **не завершится**: REST (8182) и RPC (8181) продолжат работать, WebSocket будет недоступен до освобождения порта и перезапуска.
