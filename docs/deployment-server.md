...
## 0. Перед запуском: если порты уже заняты

При ошибке **bind: address already in use** для портов 8181 (RPC), 8182 (REST), 8183 (WebSocket) остановите процесс, который их занимает.

**Linux (по номеру порта):**
```bash
# Узнать PID процесса на порту (например 8182)
sudo lsof -i :8182
# или
...
# Или одной командой (осторожно: завершит любой процесс на порту)
sudo fuser -k 8181/tcp
sudo fuser -k 8182/tcp
sudo fuser -k 8183/tcp
```

**Перед повторным запуском** убедитесь, что предыдущий экземпляр ноды остановлен (Ctrl+C, `kill $(cat gnd-node.pid)` или `systemctl stop gnd-node`).

Если занят порт **8182** (REST) или **8183** (WebSocket), нода **не завершится**: остальные сервисы (RPC, REST или WebSocket) продолжат работать. Недоступный сервис заработает после освобождения порта и перезапуска ноды.
